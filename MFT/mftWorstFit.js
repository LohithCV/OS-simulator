    ...
    for (i = 0; i < noOfPartitions; i++) {
      if (partitionOccupied[i] == 0) {
        if (process_size <= partition_size[i]) {
          if (worst_ind == -1 || (partition_size[i] > worst_size)) {
            found = 1;
            worst_ind = i;
            worst_size = partition_size[i];
          }
        }
      }
    }
    if (found == 1) {
        partitionOccupied[worst_ind] = 1;
        partition_process_id[worst_ind] = currProcessId;
        processMap.set(currProcessId,process_size)
        drawProcess(process_size, currProcessId, worst_ind);
        internalFragments+=partition_size[worst_ind]-process_size
        externalFragments-=partition_size[worst_ind]
    }
    if (found == 0 && fromQ == 0) {
