  ...
  function addProcess(process_size, process_id, fromQ) {
    ...
    if(numberOfPartition == 0) {
        ...
        for(i = 0; i < numberOfPartition; i++) {
            ...
            if(i == 0) {
                if(partition_start[0] >= process_size) {
                    worst_ind = 0;
                    worst_size = partition_start[0];
                    found = 1;
                }
            }
            else {
                if((partition_start[i] - partition_end[i-1]) >= process_size) {
                    if((partition_start[i] - partition_end[i-1]) > worst_size) {
                        worst_ind = i;
                        worst_size = partition_start[i] - partition_end[i-1];  
                        found = 1;
                    }
                }
            }
        ...
        if((total_mem_size - partition_end[numberOfPartition-1]) >= process_size) {
            ...
            if((total_mem_size - partition_end[numberOfPartition-1]) > worst_size) {
                worst_ind = numberOfPartition;
                worst_size = total_mem_size - partition_end[numberOfPartition-1];  
                found = 1;
            }
        }
        if(found == 1) {
